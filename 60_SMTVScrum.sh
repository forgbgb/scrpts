...
LOG=/home/ing01815/LOGS/${SCRUM}_Scrum_${HOSTNAME}_${DT}.log
...
TUXHOME="/Android_Reldata/official_product_releases/2K18HTV/Scrum"
...
case "$1" in
      ...
      RFBasedCloning) echo "RFBasedCloning"
        ${REPO} init -m default_Scrum_RFBasedCloning.xml   -u ssh://url/tpv/platform/manifest -b 2k18_mtk_msaf_kane_n_devprod_EU_HTV  --reference=/htv_archive/2K18HTV/MSAF_2k18_EU_HTV_M
        ;;
      *) echo "Default smtv "
        ${REPO} init -m default_Scrum_SmartTV.xml  -u ssh://url/tpv/platform/manifest -b 2k18_mtk_msaf_kane_n_devprod_EU_HTV  --reference=/htv_archive/2K18HTV/MSAF_2k18_EU_HTV_M
        ;;
...
TUXPATH="http://tux.tpvision.com/Android_Releases/official_product_releases/2K18HTV//Scrum/${SCRUM}/${DT}" 
...
if  [  $BLDSTATUS == "True"   ] ; then
  /usr/bin/mail  -s " $SCRUM  Bld Passed   $HOSTNAME " ${MAILIDS}  < ${PD}/tuxpath
else
    /usr/bin/mail -s " $SCRUM  Bld Failed  $HOSTNAME " ${MAILIDS}  < ${PD}/tuxpath 
fi 
