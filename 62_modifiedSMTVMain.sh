...
do 
 ...
 echo " ........... $CONFLICTS  $COUNT " 
  if   $CONFLICTS    ; then
   /usr/bin/mail -s " $SCRUM Mainline  Conflicts found .. stopping bld process   $HOSTNAME" ${MAILIDS}  < ${PD}/a
   $(which rm) ${filename}
    exit 0
    elif ! $COUNT  ; then
      /usr/bin/mail -s " $SCRUM Mainline No commits found .. stopping bld process   $HOSTNAME" ${MAILIDS}  < ${PD}/a
       $(which rm) ${filename}
      exit 0
    else
